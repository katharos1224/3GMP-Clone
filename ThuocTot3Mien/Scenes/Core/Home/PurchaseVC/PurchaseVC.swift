...
final class PurchaseVC: BaseViewController {
    ...
    lazy var createPaymentContentView: PaymentView = {
        let width = Constants.WIDTH_SCREEN
    ...
    @objc func keyboardWillShow(_ notification: Notification) {
        if let keyboardSize = (notification.userInfo?[UIResponder.keyboardFrameEndUserInfoKey] as? NSValue)?.cgRectValue {
            ...
            createPaymentPopupView.frame.origin.y -= keyboardSize.height * 0.35
        }
    }
    ...
    func showCreatePaymentPopup() {
        ...
        createPaymentContentView.showFailedMessage = { [weak self] failedMessage in
            self?.showAlert(title: "Thông báo", message: failedMessage)
        }
